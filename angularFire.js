...
angular.module("firebase").directive('firebase', ["$interpolate", "$parse", "angularFireResource", function($interpolate, $parse, angularFireResource) {
	return {
		...
		link: function(scope, element, attrs) {
			var domain = $interpolate(attrs.domain)(scope).replace(/^\/|\/$/g, ''),
				location = $interpolate(attrs.location)(scope).replace(/^\/|\/$/g, '');
				name;
			if (attrs.name) {
	...
	function($timeout, $q) {
		return function(url, scope, resource) {
			...
			local = {
				...
				change: function(a, b) {
					// ignore changes triggered by firebase.on('value')
					// and before initial firebase value
					if (angular.isDefined(remote.value) && !angular.equals(a, remote.value)) {
						remote.set(a, function(error) {
							if (error)
								local.set(b);
						})
					}
				},
				// wrap scope.$watch de-register method
				unWatch: function() {
					if (angular.isFunction(local._unWatch))
						local._unWatch();
				},
				// set by scope.$watch
				_unWatch: undefined
			},
			// private firebaseio utils
			remote = {
				set: function(value, cb) {
					// do not save $ decorations
					firebase.set(angular.copy(value), cb);
				},
				watch: function() {
					remote.unWatch();
					firebase.on('value', remote.change);
				},
				unWatch: function() {
					firebase.off('value', remote.change);
				},
