...
STANDALONE_DEST="../firebase-clients/libs/angularfire"
STANDALONE_STUB="angularfire"


###############################
#  VALIDATE angularfire REPO  #
###############################
# Ensure the checked out angularfire branch is master
CHECKED_OUT_BRANCH="$(git branch | grep "*" | awk -F ' ' '{print $2}')"
if [[ $CHECKED_OUT_BRANCH != "master" ]]; then
  echo "Error: Your angularfire repo is not on the master branch."
  exit 1
fi

# Make sure the angularfire branch does not have existing changes
if ! git --git-dir=".git" diff --quiet; then
  echo "Error: Your angularfire repo has existing changes on the master branch. Make sure you commit and push the new version before running this release script."
  exit 1
fi
...
PARSED_CLIENT_VERSION=$(head -2 dist/angularfire.js | tail -1 | awk -F ' ' '{print $3}')
...
README_VERSION="$(grep '<script src=\"https://cdn.firebase.com/libs/angularfire/' README.md | awk -F '/' '{print $6}')"
...
#  PUBLISH TO Bower  #
######################
# Pull any changes to the angularfire repo
git pull origin master
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git pull' from angularfire repo."
  exit 1
fi
...
git tag v$VERSION
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git tag' from angularfire repo."
  exit 1
fi
...
git push --tags
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git push --tags' from angularfire repo."
  exit 1
fi
...
git commit -am "[firebase-release] Updated AngularFire to $VERSION"
if [[ $? -ne 0 ]]; then
...
echo "  2) Update the release notes for AngularFire version ${VERSION} on GitHub"
echo "  3) Update all AngularFire client version numbers specified in firebase-website to ${VERSION}"
echo "  4) Update angularFire-seed repo"
echo "  5) Update firebase-angular-starter-pack repo"
echo "  6) Update generator-angularfire repo"
echo "  7) Submit pull request for AngularJS homepage with new version number"
echo "  8) Tweet @FirebaseRelease: 'v${VERSION} of AngularFire is available https://cdn.firebase.com/libs/angularfire/$VERSION/angularfire.min.js Changelog: https://cdn.firebase.com/libs/angularfire/changelog.txt'"
echo
echo "Done! Woot!"
echo
